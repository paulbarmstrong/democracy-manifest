...
const POLICIES: Array<Policy> = [
	...
	{name: "Foreign Trade", hue: 90, content: [0, 1, 2].map(level => <span style={{display: "flex", alignItems: "center"}}>
        {50 * (2-level)}% tariffs cause imports of <Icon name="Food" gap={3}/> to cost ${getImportPrice("Food", level)}, imports of <Icon name="Luxury" gap={3}/> to cost ${getImportPrice("Luxury", level)}, and {level} trade deal{s(level)} {isAre(level)} drawn
    </span>)},
